module Sender::ExchangeBD {
	...
	public fun initialize(exchange_acct: &signer, initializer_addr: address, comm_rate: u64, coin_b_amt: u64, coin_d_amt: u64) acquires LPCoin {
		...
		let coin_b_transferred = CoinB::transfer_between(initializer_addr, exchange_addr, coin_b_amt);
		let coin_d_transferred = CoinD::transfer_between(initializer_addr, exchange_addr, coin_d_amt);

		//Calculate the amount of LP to give to intializer (setting equal to CoinAB
		//FIXME: IDK if this is best way
		let minted_lp_coin_amt = coin_b_transferred;
		...
		move_to<Exchange>(exchange_acct, Exchange { coin_b: coin_b_transferred, 
	...
	public(script) fun initialize_exchange(exchange: signer, initializer: address, comm_rate: u64, coin_b_amt: u64, coin_d_amt: u64) acquires LPCoin {
		initialize(&exchange, initializer, comm_rate, coin_b_amt, coin_d_amt)
	}
	...
	public fun add_liquidity(coin_b_amt: u64, provider_addr: address, exchange_addr: address): u64 acquires Exchange, LPCoin {
		...
		let transferred_coin_b = CoinB::transfer_between(provider_addr, exchange_addr, coin_b_amt);
	...
	public(script) fun add_exchange_liquidity(exchange: address, provider: address, coin_b_amt: u64) acquires Exchange, LPCoin {
		add_liquidity(coin_b_amt, provider, exchange);
	}
	...
	public fun remove_liquidity(lp_coin_amt: u64, provider_addr: address, exchange_addr: address): (u64, u64) acquires Exchange, LPCoin {
		...
		let transferred_coin_b = CoinB::transfer_between(exchange_addr, provider_addr, coin_b_amt);
		exchange_obj.coin_b = exchange_obj.coin_b - transferred_coin_b;
	...
	public fun swap_coinB_to_coinD(coin_b_amt: u64, swapper_addr: address, exchange_addr: address): u64 acquires Exchange {
		//Make sure exchange exists
		assert!(exists_at(exchange_addr), 1);
		assert!(CoinB::exist_at(exchange_addr), 1);
		assert!(CoinD::exist_at(exchange_addr), 1);
		...
		let transferred_coin_b = CoinB::transfer_between(swapper_addr, exchange_addr, coin_b_amt);
		exchange_obj.coin_b = exchange_obj.coin_b + transferred_coin_b;
	...
	public(script) fun exchange_coinB_to_coinD(swapper: address, exchange: address, coin_b_amt: u64) acquires Exchange {
		swap_coinB_to_coinD(coin_b_amt, swapper, exchange);
	}

	public fun swap_coinD_to_coinB(coin_d_amt: u64, swapper_addr: address, exchange_addr: address): u64 acquires Exchange {
		//Make sure exchange exists
		assert!(exists_at(exchange_addr), 1);
		assert!(CoinB::exist_at(exchange_addr), 1);
		assert!(CoinD::exist_at(exchange_addr), 1);
		...
		let transferred_coin_b = CoinB::transfer_between(exchange_addr, swapper_addr, coin_b_amt);
		exchange_obj.coin_b = exchange_obj.coin_b - transferred_coin_b;

		transferred_coin_b
	}
	
	public(script) fun exchange_coinD_to_coinB(swapper: address, exchange: address, coin_d_amt: u64) acquires Exchange {
		swap_coinD_to_coinB(coin_d_amt, swapper, exchange);
	}
}
