module Sender::ExchangeBC {
	// Exchange module between coinC and coinC
	use Std::Signer;
	use Sender::CoinB;
	use Sender::CoinC;
	...
	public fun initialize(exchange_acct: &signer, initializer_addr: address, comm_rate: u64, coin_b_amt: u64, coin_c_amt: u64) acquires LPCoin {
	...
	public(script) fun initialize_exchange(exchange: signer, initializer: address, comm_rate: u64, coin_b_amt: u64, coin_c_amt: u64) acquires LPCoin {
		initialize(&exchange, initializer, comm_rate, coin_b_amt, coin_c_amt)
	}
	...
	public fun swap_coinB_to_coinC(coin_b_amt: u64, swapper_addr: address, exchange_addr: address): u64 acquires Exchange {
		//Make sure exchange exists
		assert!(exists_at(exchange_addr), 1);
		assert!(CoinB::exist_at(exchange_addr), 1);
		assert!(CoinC::exist_at(exchange_addr), 1);
	...
	public(script) fun exchange_coinB_to_coinC(swapper: address, exchange: address, coin_b_amt: u64) acquires Exchange {
		swap_coinB_to_coinC(coin_b_amt, swapper, exchange);
	}

	public fun swap_coinC_to_coinB(coin_c_amt: u64, swapper_addr: address, exchange_addr: address): u64 acquires Exchange {
		//Make sure exchange exists
		assert!(exists_at(exchange_addr), 1);
		assert!(CoinB::exist_at(exchange_addr), 1);
		assert!(CoinC::exist_at(exchange_addr), 1);
	...
	public(script) fun exchange_coinC_to_coinB(swapper: address, exchange: address, coin_c_amt: u64) acquires Exchange {
		swap_coinC_to_coinB(coin_c_amt, swapper, exchange);
	}
}
