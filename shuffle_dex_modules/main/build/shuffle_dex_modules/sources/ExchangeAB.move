module Sender::ExchangeAB {
	...
	public fun initialize(exchange_acct: &signer, initializer_addr: address, comm_rate: u64, coin_a_amt: u64, coin_b_amt: u64) acquires LPCoin {
	...
	public(script) fun initialize_exchange(exchange: signer, initializer: address, comm_rate: u64, coin_a_amt: u64, coin_b_amt: u64) acquires LPCoin {
		initialize(&exchange, initializer, comm_rate, coin_a_amt, coin_b_amt)
	}
	...
	public fun swap_coinA_to_coinB(coin_a_amt: u64, swapper_addr: address, exchange_addr: address): u64 acquires Exchange {
		//Make sure exchange exists
		assert!(exists_at(exchange_addr), 1);
		assert!(CoinA::exist_at(exchange_addr), 1);
		assert!(CoinB::exist_at(exchange_addr), 1);
	...
	public(script) fun exchange_coinA_to_coinB(swapper: address, exchange: address, coin_a_amt: u64) acquires Exchange {
		swap_coinA_to_coinB(coin_a_amt, swapper, exchange);
	}

	public fun swap_coinB_to_coinA(coin_b_amt: u64, swapper_addr: address, exchange_addr: address): u64 acquires Exchange {
		//Make sure exchange exists
		assert!(exists_at(exchange_addr), 1);
		assert!(CoinA::exist_at(exchange_addr), 1);
		assert!(CoinB::exist_at(exchange_addr), 1);
	...
	public(script) fun exchange_coinB_to_coinA(swapper: address, exchange: address, coin_b_amt: u64) acquires Exchange {
		swap_coinB_to_coinA(coin_b_amt, swapper, exchange);
	}
}
